...
CREATE TABLE IF NOT EXISTS `studiranje`.`st_entity` (
  ...
  `st_administrativity` SET('ROOT_ADMIN', 'GENERAL_ADMIN', 'USER', 'ENTITY', 'GROUP', 'OPTIONAL') DEFAULT 'ENTITY' NOT NULL,
  `st_estate_constraint` SET('ROOT_ADMIN', 'GENERAL_ADMIN', 'OWNER', 'USER', 'ENTITY', 'GROUP', 'OPTIONAL') DEFAULT 'ENTITY' NOT NULL,
  PRIMARY KEY (`st_identification`),
  UNIQUE INDEX `st_entity_uri_UNIQUE` (`st_entity_uri` ASC), 
  UNIQUE INDEX `st_table_UNIQUE` (`st_table_identification` ASC, `st_table_name` ASC,  `st_entity_uri` ASC)
);